...
public class Dependency2Servlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Person ps = cntx.getBean("pMgr2",PersonMgr.class).getPs(); // pMgr, pMgr2로 테스트 해보기
		
		response.setContentType("text/html; charset=utf-8");
		response.getWriter().println("이름: "+ps.getName()+"<br>");
		response.getWriter().println("나이: "+ps.getAge()+"<br>");
		response.getWriter().println("주소: "+ps.getAddr()+"<br>");
